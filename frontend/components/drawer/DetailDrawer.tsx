      ...
      {isOpen && event && (
        <>
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-40 bg-[#03040a]/58 backdrop-blur-[5px]"
            onClick={onClose}
          />
          ...
          <motion.aside
            ...
            className="fixed bottom-3 right-3 top-3 z-50 flex w-[calc(100vw-24px)] max-w-[560px] flex-col overflow-hidden rounded-[32px] border border-white/[0.09] bg-[#060814]/88 shadow-[0_32px_120px_rgba(0,0,0,0.66)] backdrop-blur-2xl md:right-5 md:top-5 md:bottom-5"
          >
            ...
            <div className="relative z-10 flex-shrink-0 px-6 pt-6 pb-5 md:px-7 md:pt-7">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  onClick={onClose}
                  className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-white/[0.08] bg-white/[0.04] text-cosmos-text-dim transition-all hover:border-cosmos-gold/20 hover:bg-cosmos-gold/10 hover:text-cosmos-text"
                  aria-label="关闭详情抽屉"
                >
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {event.tags.map((tag) => (
                  <span key={tag} className={`rounded-full px-2.5 py-1 text-[10px] font-medium ${tagClass[tag] || "border border-white/10 bg-white/5 text-cosmos-text-dim"}`}>
                    {tag}
                  </span>
                ))}
              </div>
              ...
              <div className="mt-6 rounded-3xl border border-white/[0.07] bg-white/[0.035] p-4">
                ...
                <div className="h-2 overflow-hidden rounded-full bg-black/30">
                  <motion.div
                    ...
                    className="h-full rounded-full bg-gradient-to-r from-cosmos-blue via-cosmos-gold to-cosmos-accent shadow-[0_0_24px_rgba(212,168,83,0.28)]"
                  />
                </div>
              </div>
            </div>
            ...
            <div ref={contentRef} className="relative z-10 flex-1 overflow-y-auto px-6 py-2 md:px-7">
              <div className="prose prose-invert prose-sm max-w-none
                ...
                prose-code:text-xs prose-code:font-mono prose-code:text-cosmos-accent prose-code:bg-white/[0.06] prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded
            ...
            <div className="relative z-10 flex-shrink-0 border-t border-white/[0.08] bg-black/12 px-6 py-5 md:px-7">
              <div className="mb-3 flex items-center justify-between gap-4">
                <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-cosmos-text-dim">Sources</span>
                <span className="rounded-full bg-white/[0.05] px-2 py-0.5 font-mono text-[10px] text-cosmos-text-dim">
              ...
              <div className="space-y-2">
                {event.source_urls.map((url) => (
                  <a
                    ...
                    className="block truncate rounded-2xl border border-white/[0.06] bg-white/[0.035] px-3 py-2 text-xs text-cosmos-blue/78 transition-all hover:border-cosmos-blue/30 hover:bg-cosmos-blue/10 hover:text-cosmos-blue"
                  >
                    {url}
                  </a>
                ))}
